...
## Correções de bugs (encontradas durante a refatoração)

- [x] **Teleporte da Gaia ao nascer inimigos** — o pooling adicionava o inimigo à árvore na origem (0,0) e só depois o reposicionava; nascendo sobre a Gaia (que spawnava em 0,0), a depenetração a ejetava. Corrigido: `PoolManager.spawn`/`spawn_path` aceita `at_global_position` e posiciona o nó **antes** do `add_child` (`EnemySpawner`, `DropController`, arma da Gaia passam a posição).
- [x] **Empurrão/teleporte da Gaia em aglomerados** — o corpo da Gaia tinha `EnemyBody` na `collision_mask`, então o `move_and_slide` dela resolvia a sobreposição com inimigos empurrando-a; com vários ao redor, a depenetração acumulada a teleportava (contrariando o `player_body_slide`, cuja intenção é os inimigos escorregarem ao redor dela). Corrigido: colisão **one-way** — `PlayerController._configure_enemy_body_collision()` remove `EnemyBody` da máscara da Gaia (export `collide_with_enemy_bodies`, default `false`); os inimigos continuam detectando-a e escorregando.
...
- [x] **[Alta] Sem object pooling** — **FEITO:** criado `autoloads/PoolManager.gd` (autoload), pool central com **filas agrupadas por cena** e instâncias inativas guardadas **fora da árvore** (zero processamento/colisão). API `spawn`/`spawn_path`/`despawn`/`prewarm`/`clear_all` + hooks `_on_pool_acquire()`. Ligados: inimigos (`EnemySpawner`+`EnemyBase`, com `prewarm_pool_count` no spawner), moedas (`DropController`+`CoinDrop`), hitbox de ataque (`GaiaInitialWeaponController`+`DirectionalAttackHitbox`), visual de ataque (`GaiaAttackVisualController`) e texto flutuante (`WorldFeedbackLayer`+`FloatingCombatText`, com `animation_started` resetado no reuso). Morte/coleta/expiração agora chamam `PoolManager.despawn` (fallback `queue_free` se não-poolado).
- [x] **[Alta] `load()` em runtime a cada spawn** — **FEITO:** o `PoolManager.get_scene()` cacheia cada `PackedScene` por caminho; todos os call-sites de spawn (`spawn_path`) usam o cache, eliminando `load()` por moeda/inimigo/ataque.
- [x] **[Alta] `is_gameplay_blocked()` resolvido por frame via grupo + reflexão** — **FEITO (removido em vez de cacheado):** o sistema era redundante com a pausa nativa do Godot. A pausa de gameplay (level-up, fim de run) já usa `get_tree().paused` (`RunController`) + `process_mode = ALWAYS` na UI; quando a árvore está pausada os `_process`/`_physics` das entidades nem rodam, tornando a checagem desnecessária. Removidas as 14 chamadas `RunQuery.is_gameplay_blocked` de `EnemyBase`, `EnemyAttackHitbox`, `CoinDrop`, `DirectionalAttackHitbox`, `PlayerDashImpactArea`, `EnemySpawner`, `GaiaInitialWeaponController` e `PlayerController` (a arma/player já param via `is_alive`). `is_gameplay_blocked`/`is_run_paused` removidos do `RunQuery` (mantidos `is_run_ending`/`is_run_finished`, usados só no callback do `DropController`). Decisão de design: durante o delay de derrota (0.75s) **o mundo segue** rodando e só pausa no ResultPanel.
- [x] **[Alta] `queue_redraw()` por frame mesmo com debug desligado** — **FEITO:** criado um helper `_queue_debug_redraw()` em `EnemyBase` (flag `draw_debug_visual`), `PlayerController` (`draw_debug_aim`) e `CoinDrop` (`draw_debug_visual`) que só chama `queue_redraw()` quando a flag-mestra do `_draw` está ligada. Todas as chamadas por-frame (6/7/4 respectivamente) passaram a usar o helper — com debug off (caso comum) nenhum canvas é marcado dirty. `DirectionalAttackHitbox`/`TestArena` chamam `queue_redraw()` apenas no setup (one-shot), não por frame — deixados como estão.
- [x] **[Alta] Falta `class_name` → reflexão nos caminhos de dano** — **FEITO:** declarados `class_name PlayerController` e `class_name EnemyBase`. Reflexão removida dos caminhos de dano/dash, trocada por cast tipado (`as EnemyBase` / `as PlayerController`) + chamada direta: `EnemyAttackHitbox` (player), `DirectionalAttackHitbox` (inimigo, + knockback), `PlayerDashImpactArea` (inimigo, dano + knockback) e `GaiaInitialWeaponController` (`player_controller: PlayerController`, `_resolve_player_controller` via `is PlayerController`, regras de dash diretas). Removidos os 2 helpers `_variant_to_damage` que ficaram órfãos. **Mantido de propósito:** `HurtboxComponent.has_method("receive_damage")` — é componente genérico de combate, não deve acoplar a tipos do jogo.
- [x] **[Alta] Contagem de inimigos vivos varre grupo** — **FEITO:** `_get_alive_enemy_count()` agora retorna um contador incremental `_alive_enemy_count` em O(1) (era `get_nodes_in_group("enemy")` + iteração a cada spawn e 3× no log). Incrementa no `force_spawn_enemy` após spawn bem-sucedido; decrementa no callback `_on_enemy_died_count` (signal global `enemy_died`, disparado no `die()` que também sai do grupo — mesma semântica de antes), com `max(0, ...)` de proteção. Premissa: 1 spawner por cena (caso atual) — o contador é por-spawner.
